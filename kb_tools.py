...
def _read_one(filename: str, start_line: int, end_line: int) -> str:
    try:
...
@tool
def read_document(filenames: str, start_line: int = 1, end_line: int = MAX_READ_LINES) -> str:
    """Read one or more documents in the knowledge base. Called with just a filename,
    this returns the ENTIRE document (up to 400 lines) — prefer this by default so your
    answer is grounded in the full article, not just the snippet search_documents matched
    on. Only set start_line/end_line to jump to a specific section, and only when you
    already know the document is longer than 400 lines. Pass several filenames
...
KB_TOOLS = [list_documents, find_documents, search_documents, read_document]    cap = min(max_results, MAX_RESULTS)
    results: list[str] = []
    for path in _docs():
        lines = path.read_text(encoding="utf-8", errors="replace").splitlines()
        for i, line in enumerate(lines):
